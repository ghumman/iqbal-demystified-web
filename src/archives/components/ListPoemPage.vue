...
<table>
  <div v-for="(section,index) in poemList.sections"> 
    ...
    <div v-for="(poem,index) in section.poems">
      ...
      <div v-if=" index % 2 == 0">
      <router-link :to="{ path: 'PoemPage', query: { poemId: poem.id , messageFromSigninPage:profileSigninMessage}}"><tr><td><even_tab_color><tab_title class="a">{{ poem.poemName[0].text }}</tab_title></br></br>( {{ poem.poemName[1].text }} )</br></br></br> </even_tab_color></td></tr></router-link>
      </div>

      <div v-else>
	<router-link :to="{ path: 'PoemPage', query: { poemId: poem.id , messageFromSigninPage:profileSigninMessage}}"><tr><td><odd_tab_color><tab_title class="a">{{ poem.poemName[0].text }}</tab_title> </br></br>( {{ poem.poemName[1].text }} )</br></br></br> </odd_tab_color></td></tr></router-link>
      </div>
    </div>
